...
public class ReplyService {
    ...
    // 댓글 목록 전체조회
    public ReplyListDto getReplies(long boardId, Page page) {
        List<Reply> replies = replyMapper.findAll(boardId, page);
        List<ReplyResponseDetailDto> dtoList = replies.stream()
                .map(r -> new ReplyResponseDetailDto(r))
                .collect(Collectors.toList());

        return ReplyListDto.builder()
                .replies(dtoList) // 댓글 목록
                .pageInfo(new PageMaker(page, replyMapper.count(boardId))) // 페이지 정보
    ...
    @Transactional
    public ReplyListDto remove(long replyId) {
        // 댓글 번호로 원본 글번호 찾기
        long boardId = replyMapper.findBno(replyId);
        boolean flag = replyMapper.delete(replyId);

        // 댓글 삭제 후 삭제된 댓글 목록을 반환
        return flag ? getReplies(boardId, new Page(1, 10)) : null;
    }

    // 댓글 수정
    public ReplyListDto modify(ReplyRequestModifyDto dto) {
        // dto -> entity
        // -> 데이터가 클라이언트에서 데이터베이스에 저장되도록 하기 위해
        replyMapper.modify(dto.toEntity());
        // 수정 후 댓글 목록 반환 (수정사항 반영)
        return getReplies(dto.getBoardId(), new Page(1, 10));
    }
